...
the preceding 38 minutes. Take no number from `/rooms` unless the response says
`cf-cache-status: MISS`: it is edge-cached at `s-maxage=86400`, arbitrary query parameters are not
in the cache key, and at 05:4xZ three separately aged copies were being served at once, two of
them still carrying the pre-raise capacity. `/config` is the endpoint to read. `rate_rooms_per_day` stayed 20 in every reading, and by 04:26Z this
